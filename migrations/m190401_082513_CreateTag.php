...
/**
 * Class m190401_082513_CreateTag
 */
class m190401_082513_CreateTag extends Migration
{
    /**
     * {@inheritdoc}
     */
...
//            $tableOptions = 'CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE=InnoDB';
        ...
        $this->createIndex('idx-primary', '{{%tag_edge}}', ['src', 'type', 'dst'], true);
        $this->createIndex('idx-key_dst', '{{%tag_edge}}', ['dst', 'type', 'src'], true);
        $this->createIndex('idx-src', '{{%tag_edge}}', ['src', 'type', 'created_at', 'seq'], true);


        $this->createTable('{{%tag_edgedata}}', [
            'id' => $this->primaryKey(),
        ...
        $this->createTable('{{%tag_transactions}}', [
            'id' => $this->primaryKey(),
            'phid' => $this->string(64)->notNull()->unique(),
        ...
        $this->createIndex('idx-object_phid', '{{%tag_transactions}}', 'object_phid');
    }
    ...
    {
        $this->dropTable('{{%tag}}');
        $this->dropTable('{{%tag_edge}}');
        $this->dropTable('{{%tag_edgedata}}');
        $this->dropTable('{{%tag_transactions}}');
    }
    ...
    {
        echo "m190401_082513_CreateTag cannot be reverted.\n";
