...
use orangins\modules\herald\assets\JavelinHeraldBehaviorAsset;
...
use orangins\modules\herald\engine\exception\HeraldInvalidActionException;
...
use orangins\modules\herald\systemaction\HeraldActionGroup;
use orangins\modules\herald\value\HeraldEmptyFieldValue;
use orangins\modules\herald\value\HeraldFieldValue;
use orangins\modules\herald\xaction\HeraldRuleEditTransaction;
use orangins\modules\herald\xaction\HeraldRuleNameTransaction;
...
/**
 * Class HeraldRuleController
 * @package orangins\modules\herald\actions
 * @author 陈妙威
 */
final class HeraldRuleAction extends HeraldController
{
    ...
    public function run()
    {
        ...
        if ($id) {
            ...
            $cancel_uri = $this->getApplicationURI('index/view', ['id' => $rule->getID()]);
        } else {
            $new_uri = $this->getApplicationURI('index/new');
...
//                $uri = '/' . $rule->getMonogram();
                $uri = $this->getApplicationURI('index/view', ['id' => $rule->getID()]);
...
//        require_celerity_resource('herald-css');
        ...
        if ($rule->isObjectRule()) {
            $trigger_object_control = (new AphrontFormStaticControl())
                ->setValue(
                    pht(
        ...
        $form
            ...
            ->appendChild(
                (new PHUIFormInsetView())
                    ...
                    ->setRightButton(JavelinHtml::phutil_tag(
                        'a',
                        array(
                            'href' => '#',
                    ...
                    ->setContent(JavelinHtml::phutil_tag(
                        ...
                        array(
                            ...
                            'class' => 'w-100 herald-condition-table',
                        ),
                        '')))
            ->appendChild(
                (new PHUIFormInsetView())
                    ...
                    ->setContent(JavelinHtml::phutil_tag(
                        ...
                        array(
                            'sigil' => 'rule-actions',
                            'class' => 'w-100 herald-action-table',
                        ),
                        '')))
            ->appendChild(
    ...
    private function saveRule(HeraldAdapter $adapter, $rule, $request)
    {
        ...
        if (!$errors) {
            $new_state = (new HeraldRuleSerializer())->serializeRuleComponents(
                $match_all,
                $conditions,
                $actions,
                $repetition_policy);
            ...
            if ($is_new) {
                $xactions[] = (new HeraldRuleTransaction())
        ...
        HeraldAdapter $adapter)
    {
        ...
        JavelinHtml::initBehavior(
            new JavelinHeraldBehaviorAsset(),
            array(
    ...
    private function loadRulesThisRuleMayDependUpon(HeraldRule $rule)
    {
        ...
        /** @var HeraldRule[] $all_rules */
        $all_rules = HeraldRule::find()
            ->setViewer($viewer)
