...
class ConduitTokenPHIDType extends PhabricatorPHIDType
{
    const TYPECONST = "CONT";
    /**
     * @return mixed
     * @author 陈妙威
     */
     ...
     * @return PhabricatorPolicyAwareQuery|PhabricatorConduitTokenQuery
    ...
    protected function buildQueryForObjects(PhabricatorObjectQuery $query,
                                            array $phids)
    {
        return  PhabricatorConduitToken::find()->withPHIDs($phids);
    }
    ...
    {
        foreach ($handles as $phid => $handle) {
            /** @var PhabricatorConduitToken $user */
            $user = $objects[$phid];
            $handle
                ->setName($user->token_type)
                ->setFullName($user->token);
        }
    }
}