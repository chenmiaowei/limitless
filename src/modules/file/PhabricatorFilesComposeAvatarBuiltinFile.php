...
/**
 * Class PhabricatorFilesComposeAvatarBuiltinFile
 * @package orangins\modules\file
 * @author 陈妙威
 */
final class PhabricatorFilesComposeAvatarBuiltinFile extends PhabricatorFilesBuiltinFile
{
     ...
     * @throws exception\PhabricatorFileStorageConfigurationException
     * @author 陈妙威
     */
    public function updateUser(PhabricatorUser $user)
    {
        ...
        $initial = OranginsUtf8::phutil_utf8_strtoupper(substr($username, 0, 1));
...
//        unset($unguarded);
        ...
        if (!isset($this->maps[$map_key])) {
            switch ($map_key) {
                ...
                case 'border':
                    ...
                    break;
                default:
                    throw new Exception(\Yii::t("app",'Unknown map "%s".', $map_key));
            }
    ...
    private function composeImage($color, $image, $border)
    {
        ...
        if (!function_exists('imagecreatefromstring')) {
            ...
            return FileSystemHelper::readFile($default_path);
        }

        $color_const = hexdec(trim($color, '#'));
        $true_border = self::rgba2gd($border);
        $image_map = $this->getMap('image');
        $data = FileSystemHelper::readFile($image_map[$image]);
        ...
        if (!$image_fill) {
            throw new Exception(
                \Yii::t("app",'Failed to save builtin avatar image data (imagefill).'));
        }

        $border_thickness = imagesetthickness($canvas, 64);
        if (!$border_thickness) {
            throw new Exception(
                \Yii::t("app",'Failed to save builtin avatar image data (imagesetthickness).'));
        }

        $image_rectangle = imagerectangle($canvas, 0, 0, 400, 400, $true_border);
        if (!$image_rectangle) {
            throw new Exception(
                \Yii::t("app",'Failed to save builtin avatar image data (imagerectangle).'));
        }

        $image_copy = imagecopy($canvas, $img, 0, 0, 0, 0, 400, 400);
        if (!$image_copy) {
            throw new Exception(
                \Yii::t("app",'Failed to save builtin avatar image data (imagecopy).'));
        }
