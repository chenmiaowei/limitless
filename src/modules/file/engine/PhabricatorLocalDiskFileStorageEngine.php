 ...
 */
final class PhabricatorLocalDiskFileStorageEngine extends PhabricatorFileStorageEngine
{
    ...
    private function getLocalDiskFileStorageRoot()
    {
        ...
        if (!$root || $root == '/' || $root[0] != '/') {
            throw new PhabricatorFileStorageConfigurationException(\Yii::t("app",
                    "Malformed local disk storage root. You must provide an absolute " .
                    "path, and can not use '{0}' as the root.", [
                        '/'
                    ]));
        }
