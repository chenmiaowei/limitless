...
/**
 * Class PhabricatorFileThumbnailTransform
    ...
    extends PhabricatorFileImageTransform
{
    ...
    {
        return array(
            (new PhabricatorFileThumbnailTransform())
                ->setName(\Yii::t("app","Profile (400px \xC3\x97 400px)"))
                ->setKey(self::TRANSFORM_PROFILE)
                ->setDimensions(400, 400)
                ->setScaleUp(true),
            (new PhabricatorFileThumbnailTransform())
                ->setName(\Yii::t("app","Pinboard (280px \xC3\x97 210px)"))
                ->setKey(self::TRANSFORM_PINBOARD)
                ->setDimensions(280, 210),
            (new PhabricatorFileThumbnailTransform())
                ->setName(\Yii::t("app",'Thumbgrid (100px)'))
                ->setKey(self::TRANSFORM_THUMBGRID)
                ->setDimensions(100, null),
            (new PhabricatorFileThumbnailTransform())
                ->setName(\Yii::t("app",'Preview (220px)'))
                ->setKey(self::TRANSFORM_PREVIEW)
                ->setDimensions(220, null),
            (new self())
                ->setName(\Yii::t("app",'Workcard (526px)'))
