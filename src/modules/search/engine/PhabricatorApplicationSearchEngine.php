...
use orangins\modules\search\field\PhabricatorSearchOrderField;
 ...
 */
abstract class PhabricatorApplicationSearchEngine extends OranginsObject
{
    ...
    public function buildResponse()
    {
        ...
        $search = (new PhabricatorApplicationSearchAction('search', $this->getAction()->controller))
            ->setQueryKey($request->getURIData('queryKey'))
            ->setSearchEngine($this);

        return $action->delegateToAction($search);
    }
    ...
    public function saveQuery(PhabricatorSavedQuery $query)
    {
        ...
        $query->setEngineClassName($this->getClassShortName());

//        $unguarded = AphrontWriteGuard::beginScopedUnguardedWrites();
        try {
            if (!$query->save()) {
                throw new ActiveRecordException("PhabricatorSavedQuery create error.", $query->getErrorSummary(true));
            }
...
//        unset($unguarded);
     ...
     *
     * @param PhabricatorSavedQuery $original
     * @throws Exception
     * @throws PhutilInvalidStateException
     * @throws PhutilMethodNotImplementedException
     ...
     *
     * @return array<string> Default ordering for field keys.
     */
     ...
     *
     * @return array<string> Fields to hide.
     */
     ...
     * @task uri
     */
    public function getQueryResultsPageURI($queryKey)
    {
     ...
     * @author 陈妙威
     */
    public function getExportURI($queryKey)
    {
        return $this->getURI('query', [
            'queryKey' => $queryKey,
            'queryAction' => 'export'
     ...
     * @task uri
     */
    abstract protected function getURI($path = null, $params = []);
    ...
    public function newSavedQuery()
    {
        return (new PhabricatorSavedQuery())
            ->setEngineClassName($this->getClassShortName());
    }

    /**
     * @param PHUIListView $menu
     * @return $this
     * @throws PhutilInvalidStateException
     * @throws Exception
     * @throws ReflectionException
    ...
    public function addNavigationItems(PHUIListView $menu)
    {
        ...
        foreach ($named_queries as $query) {
            ...
            $menu
                ->newLink($query->getQueryName(), $uri, 'query/' . $key)
                ->setIcon("fa-search");
        }
    ...
    public function loadAllNamedQueries()
    {
        ...
        if ($this->namedQueries === null) {
            ...
            $named_queries = PhabricatorNamedQuery::find()
                ->setViewer($viewer)
                ->withEngineClassNames(array($classShortName))
                ->withUserPHIDs(
                    array(
                        $viewer->getPHID(),
 ...
 * @throws Exception
    ...
    public function getDefaultQueryKey()
    {
        ...
        $configs = PhabricatorNamedQueryConfig::find()
            ->setViewer($viewer)
            ->withEngineClassNames(array($this->getClassShortName()))
            ->withScopePHIDs(
                array(
                    $viewer->getPHID(),
        ...
        /** @var PhabricatorNamedQueryConfig[] $configs */
        ...
        PhabricatorSavedQuery $saved)
    {
        ...
        $datasource = (new PhabricatorProjectLogicalDatasource())
    ...
    {
        return $this->getApplication()->getApplicationURI($path, $params);
    }
    ...
    protected function getApplication()
    {
        if (!$this->application) {
            ...
            $this->application = (new PhabricatorApplicationQuery())
                ->setViewer($this->requireViewer())
                ->withShortName(false)
     ...
     *
     * @return PhabricatorApplicationSearchEngine[] All available engines.
     * @task construct
     */
    ...
    {
        $allEngines = self::getAllEngines();
        return ArrayHelper::getValue($allEngines, $class_name);
    }
    ...
    public function getBuiltinQueries()
    {
        ...
        foreach ($names as $key => $name) {
            $queries[$key] = (new PhabricatorNamedQuery())
    ...
    public function getBuiltinQuery($query_key)
    {
        ...
        return ArrayHelper::getValue($this->getBuiltinQueries(), $query_key);
    }
...
//                PhabricatorProjectProjectPHIDType::TYPECONST,
        ...
        array $allow_types = array())
    {
        ...
        if ($allow_types) {
            ...
            foreach ($list as $key => $phid) {
                if (empty($allow_types[PhabricatorPHID::phid_get_type($phid)])) {
                    unset($list[$key]);
                }
            }
        }
        ...
        $end_name)
    {
        ...
        $form
            ->appendChild(
                (new PHUIFormFreeformDateControl())
                    ->setName($start_key)
                    ->setLabel($start_name)
                    ->setValue($start_str))
            ->appendChild(
    ...
    public function getResultBucket(PhabricatorSavedQuery $saved)
    {
        ...
        $buckets = $this->newResultBuckets();
        return ArrayHelper::getValue($buckets, $key);
    }
    ...
    /**
     * @param PhabricatorPolicyAwareQuery|PhabricatorCursorPagedPolicyAwareQuery $query
     * @param AphrontView|AphrontCursorPagerView $pager
     * @return array
     * @throws PhutilInvalidStateException
     * @throws Exception
     * @author 陈妙威
    ...
    /**
     * @return ConduitResultSearchEngineExtension[]
     * @throws PhutilInvalidStateException
     * @author 陈妙威
     */
    private function getConduitFieldExtensions()
    {
        /** @var ConduitResultSearchEngineExtension[] $extensions */
        $extensions = $this->getEngineExtensions();
        $object = $this->newResultObject();
        ...
        array $extension_data)
    {
        ...
        $fields = array();
        foreach ($field_extensions as $key => $extension) {
            $data = ArrayHelper::getValue($extension_data, $key, array());
...
//            (new PhabricatorIDExportField())
//                ->setKey('id')
//                ->setLabel(\Yii::t("app", 'ID')),
//        );
//
//        if ($object instanceof ActiveRecordPHID) {
//            $builtin_fields[] = (new PhabricatorPHIDExportField())
//                ->setKey('phid')
...
//        $fields = mpull($builtin_fields, null, 'getKey');
    ...
    {
        $object = $this->newResultObject();
        $has_phid = $object instanceof ActiveRecordPHID;
