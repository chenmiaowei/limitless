...
JX.behavior('phabricator-search-typeahead', function (config) {
    ...
    function updateIcon(button, data, new_icon) {
        var icon = JX.DOM.find(button, 'i', 'global-search-dropdown-icon');
    ...
    JX.Stratcom.listen('click', 'global-search-dropdown', function (e) {
        ...
        function updateValue(spec) {
            ...
            var req =  new JX.Request(config.scopeUpdateURI);
            var list_of_pairs = [];
            list_of_pairs.push([$('meta[name=csrf-param]').attr('content'), $('meta[name=csrf-token]').attr('content')]);
            list_of_pairs.push(['value', data.value]);
            req.setDataWithListOfPairs(list_of_pairs);
            req.send();
        }
