...
/**
 * Class PhabricatorSearchResultView
 * @package orangins\modules\search\view
 * @author 陈妙威
 */
...
//        require_celerity_resource('phabricator-search-results-css');
        ...
        $type_name = nonempty($handle->getTypeName(), \Yii::t("app", 'Document'));
        ...
        if ($handle->getStatus() == PhabricatorObjectHandle::STATUS_CLOSED) {
            ...
            $item->addAttribute(\Yii::t("app", 'Closed'));
        }
    ...
    private function emboldenQuery($str)
    {
        ...
        foreach ($parts as $part) {
            if ($part['highlighted']) {
                $result[] = JavelinHtml::phutil_tag('strong', array(), $part['text']);
            } else {
                $result[] = $part['text'];
            }
        }
