    ...
    extends PhabricatorApplicationConfigOptions
{
    ...
    {
        return \Yii::t('app', 'Cluster Setup');
    }

    public function getDescription()
    {
        return \Yii::t('app', 'Configure Phabricator to run on a cluster of hosts.');
    }
    ...
    public function getOptions()
    {
        ...
        $databases_help = $this->deformat(\Yii::t('app', <<<EOTEXT
...
//    $intro_href = PhabricatorEnv::getDoclink('Clustering Introduction');
        $intro_name = \Yii::t('app', 'Clustering Introduction');

        $search_type = 'cluster.search';
        $search_help = $this->deformat(\Yii::t('app', <<<EOTEXT
...
//    PhabricatorEnv::getDoclink('Cluster: Search'),
        ...
        return array(
            $this->newOption('cluster.addresses', 'list<string>', array())
                ->setLocked(true)
                ->setSummary(\Yii::t('app', 'Address ranges of cluster hosts.'))
                ->setDescription(
                    \Yii::t('app',
                ...
                ->addExample(
                    ...
                    \Yii::t('app', 'Whitelist Specific Addresses'))
                ->addExample(
                    array(
                        '1.2.3.0/24',
                    ),
                    \Yii::t('app', 'Whitelist 1.2.3.*'))
                ->addExample(
                    array(
                        '1.2.0.0/16',
                    ),
                    \Yii::t('app', 'Whitelist 1.2.*.*'))
                ->addExample(
                    array(
                        '0.0.0.0/0',
                    ),
                    \Yii::t('app', 'Allow Any Host (Insecure!)')),
            $this->newOption('cluster.instance', 'string', null)
                ->setLocked(true)
                ->setSummary(\Yii::t('app', 'Instance identifier for multi-tenant clusters.'))
                ->setDescription(
                    \Yii::t('app',
            ...
            $this->newOption('cluster.databases', $databases_type, array())
                ->setHidden(true)
                ->setSummary(
                    \Yii::t('app', 'Configure database read replicas.'))
                ->setDescription($databases_help),
            $this->newOption('cluster.search', $search_type, array())
                ->setLocked(true)
                ->setSummary(
                    \Yii::t('app', 'Configure full-text search services.'))
