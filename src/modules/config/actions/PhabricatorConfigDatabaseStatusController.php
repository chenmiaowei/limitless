    ...
    extends PhabricatorConfigDatabaseAction
{
    ...
    public function run()
    {
        ...
        if ($this->ref !== null) {
            $server_actual = ArrayHelper::getValue($actual, $this->ref);
            if (!$server_actual) {
    ...
    private function buildResponse($title, $body)
    {
        ...
        if (!$title) {
            $title = \Yii::t("app", 'Database Status');
        }
        ...
        array $actual_servers)
    {
        ...
        $properties = $this->buildProperties(
            array(),
            $comp->getIssues());
        $properties = $this->buildConfigBoxView(\Yii::t("app", 'Properties'), $properties);
        $table = $this->buildConfigBoxView(\Yii::t("app", 'Database'), $table);
        ...
        $database_name)
    {
        ...
        $properties = $this->buildProperties(
            array(
                ...
                array(
                    \Yii::t("app", 'Character Set'),
                    $actual_charset,
                ),
                array(
                    \Yii::t("app", 'Expected Character Set'),
                    $expect_charset,
                ),
                array(
                    \Yii::t("app", 'Collation'),
        ...
        $table_name)
    {
        ...
        $table_view = (new AphrontTableView($rows))
            ->setHeaders(
                array(
                    ...
                    \Yii::t("app", 'Column'),
                    \Yii::t("app", 'Data Type'),
                    \Yii::t("app", 'Column Type'),
                    \Yii::t("app", 'Nullable'),
                    \Yii::t("app", 'Autoincrement'),
                    \Yii::t("app", 'Character Set'),
                    \Yii::t("app", 'Collation'),
                ))
            ->setColumnClasses(
        ...
        $keys_view = (new AphrontTableView($key_rows))
            ->setHeaders(
                array(
                    null,
                    \Yii::t("app", 'Key'),
        ...
        $title = \Yii::t("app", '%s.%s', $database_name, $table_name);
        ...
        $box_header = \Yii::t("app", '%s.%s', $database_name, $table_name);

        $properties = $this->buildConfigBoxView(\Yii::t("app", 'Properties'), $properties);
        $table = $this->buildConfigBoxView(\Yii::t("app", 'Database'), $table_view);
        $keys = $this->buildConfigBoxView(\Yii::t("app", 'Keys'), $keys_view);
        ...
        $column_name)
    {
        ...
        $properties = $this->buildProperties(
            array(
                ...
                array(
                    \Yii::t("app", 'Expected Column Type'),
                    $expect_coltype,
                ),
                array(
                    \Yii::t("app", 'Character Set'),
                ...
                array(
                    \Yii::t("app", 'Expected Autoincrement'),
    ...
    private function buildProperties(array $properties, array $issues)
    {
        ...
        if (!$issues) {
            $status_view->addItem(
                (new PHUIStatusItemView())
                    ...
                    ->setTarget(\Yii::t("app", 'No Schema Issues')));
        } else {
        ...
        $view->addProperty(\Yii::t("app", 'Schema Status'), $status_view);
