...
final class PhabricatorConfigTableSchema
  ...
  public function addColumn(PhabricatorConfigColumnSchema $column) {
    ...
    if (isset($this->columns[$key])) {
      throw new Exception(
        \Yii::t("app",'Trying to add duplicate column "%s"!', $key));
    }
  ...
  public function addKey(PhabricatorConfigKeySchema $key) {
    ...
    if (isset($this->keys[$name])) {
      throw new Exception(
        \Yii::t("app",'Trying to add duplicate key "%s"!', $name));
    }
    $key->setTable($this);
    $this->keys[$name] = $key;
    return $this;
  ...
  public function getColumn($key) {
    return ArrayHelper::getValue($this->getColumns(), $key);
  }
  ...
  public function getKey($key) {
    return ArrayHelper::getValue($this->getKeys(), $key);
  }
  ...
  public function getPersistenceTypeDisplayName() {
    $map = array(
      self::PERSISTENCE_DATA => \Yii::t("app",'Data'),
      self::PERSISTENCE_CACHE => \Yii::t("app",'Cache'),
      self::PERSISTENCE_INDEX => \Yii::t("app",'Index'),
    );
