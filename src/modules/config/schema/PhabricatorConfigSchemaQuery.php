...
/**
 * Class PhabricatorConfigSchemaQuery
 * @package orangins\modules\config\schema
 * @author 陈妙威
 */
final class PhabricatorConfigSchemaQuery extends OranginsObject
{
    ...
    private function getAPI(PhabricatorDatabaseRef $ref)
    {
        ...
        return (new PhabricatorStorageManagementAPI())
            ->setUser($ref->getUser())
            ->setHost($ref->getHost())
            ->setPort($ref->getPort())
    ...
    /**
     * @param PhabricatorDatabaseRef $ref
     * @return PhabricatorConfigServerSchema
     * @author 陈妙威
     */
    private function loadActualSchemaForServer(PhabricatorDatabaseRef $ref)
    {
        ...
        foreach ($tables as $database_name => $database_tables) {
            ...
            $database_schema = (new PhabricatorConfigDatabaseSchema())
                ->setName($database_name)
                ->setCharacterSet($info['DEFAULT_CHARACTER_SET_NAME'])
                ->setCollation($info['DEFAULT_COLLATION_NAME']);

            $database_column_info = ArrayHelper::getValue($column_info, $database_name, array());
            $database_column_info = igroup($database_column_info, 'TABLE_NAME');

            foreach ($database_tables as $table) {
                $table_name = $table['TABLE_NAME'];

                $table_schema = (new PhabricatorConfigTableSchema())
                    ->setName($table_name)
                    ->setCollation($table['TABLE_COLLATION'])
                    ->setEngine($table['ENGINE']);

                $columns = ArrayHelper::getValue($database_column_info, $table_name, array());
                foreach ($columns as $column) {
                    ...
                    $column_schema = (new PhabricatorConfigColumnSchema())
                ...
                foreach ($keys as $key_name => $key_pieces) {
                    ...
                    $key_schema = (new PhabricatorConfigKeySchema())
                        ->setName($key_name)
        ...
        foreach ($invisible_databases as $database_name) {
            $server_schema->addDatabase(
                (new PhabricatorConfigDatabaseSchema())
                    ->setName($database_name)
    ...
    public function loadExpectedSchemaForServer(PhabricatorDatabaseRef $ref)
    {
        ...
        $specs = (new PhutilClassMapQuery())
            ->setAncestorClass(PhabricatorConfigSchemaSpec::className())
            ->execute();
    ...
    /**
     * @param array $expect_servers
     * @param array $actual_servers
     * @return array
     * @author 陈妙威
     */
    ...
    /**
     * @param PhabricatorConfigServerSchema $expect
     * @param PhabricatorConfigServerSchema $actual
     * @return PhabricatorConfigServerSchema
     * @author 陈妙威
     * @throws Exception
     */
    ...
    /**
     * @param PhabricatorConfigStorageSchema|null $expect
     * @param PhabricatorConfigStorageSchema|null $actual
     * @return array
     * @author 陈妙威
     * @throws Exception
     */
        ...
        PhabricatorConfigStorageSchema $actual = null)
    {
        ...
        if (!$expect && !$actual) {
            throw new Exception(\Yii::t("app", 'Can not compare two missing schemata!'));
