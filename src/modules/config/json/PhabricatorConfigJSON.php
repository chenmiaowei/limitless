...
namespace orangins\modules\config\json;

use PhutilJSON;
use orangins\lib\OranginsObject;

final class PhabricatorConfigJSON extends OranginsObject
{
    /**
     * Properly format a JSON value.
     *
     * @param object Any value, but should be a raw value, not a string of JSON.
     * @return string
     */
    public static function prettyPrintJSON($value)
    {
        ...
        switch ($type) {
            case 'list':
                $result = (new PhutilJSON())->encodeAsList($value);
                break;
            case 'object':
                $result = (new PhutilJSON())->encodeFormatted($value);
                break;
            default:
