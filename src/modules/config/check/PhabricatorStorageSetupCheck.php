...
final class PhabricatorStorageSetupCheck extends PhabricatorSetupCheck
{
    ...
    protected function executeChecks()
    {
        ...
        if (!$chunk_engine_active) {
            ...
            $message = \Yii::t("app",
                ...
                phutil_tag(
                    ...
                    \Yii::t("app", 'Configuring File Storage')));

            $this
                ->newIssue('large-files')
                ->setShortName(\Yii::t("app", 'Large Files'))
                ->setName(\Yii::t("app", 'Large File Storage Not Configured'))
                ->setMessage($message);
        }
        ...
        if ($post_max_size && ((int)$post_max_size > 0)) {
            ...
            if ($post_max_need > $post_max_bytes) {
                ...
                $this
                    ...
                    ->setName(\Yii::t("app", 'PHP post_max_size Not Configured'))
                    ->setSummary($summary)
                    ->setMessage($message)
        ...
        if ($memory_limit && ((int)$memory_limit > 0)) {
            ...
            if ($need_bytes > $available_bytes) {
                ...
                $this
                    ...
                    ->setName(\Yii::t("app", 'Memory Limit Restricts File Uploads'))
                    ->setSummary($summary)
                    ->setMessage($message)
                    ->setGroup(self::GROUP_PHP)
        ...
        if (!Filesystem::pathExists($local_path) ||
            ...
            $this
                ...
                ->setShortName(\Yii::t("app", 'Local Disk Storage'))
                ->setName(\Yii::t("app", 'Local Disk Storage Not Readable/Writable'))
                ->setMessage($message)
    ...
    private function checkS3()
    {
        ...
        $this->newIssue('storage.s3.partial-config')
            ->setShortName(\Yii::t("app", 'S3 Partially Configured'))
            ->setName(\Yii::t("app", 'Amazon S3 is Only Partially Configured'))
            ->setMessage($message)
