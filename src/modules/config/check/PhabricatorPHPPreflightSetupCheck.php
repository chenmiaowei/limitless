  ...
  protected function executeChecks() {
    if (version_compare(phpversion(), 7, '>=') &&
      ...
      $this->newIssue('php.version7')
        ->setIsFatal(true)
        ->setName(\Yii::t("app",'PHP 7.0 Not Supported'))
        ->setMessage($message)
        ->addLink(
          'https://phurl.io/u/php7',
          \Yii::t("app",'Phabricator PHP 7 Compatibility Information'));
    ...
    if ($safe_mode) {
      ...
      $this->newIssue('php.safe_mode')
        ->setIsFatal(true)
        ->setName(\Yii::t("app",'Disable PHP %s', 'safe_mode'))
        ->setMessage($message)
      ...
      if ($disable_value) {
        ...
        if ($fatal) {
          ...
          $this->newIssue('php.'.$disable_option)
            ->setIsFatal(true)
            ->setName(\Yii::t("app",'Remove PHP %s', $disable_option))
            ->setMessage($message)
    ...
    if ($func_overload) {
      ...
      $this->newIssue('php'.$overload_option)
        ->setIsFatal(true)
        ->setName(\Yii::t("app",'Disable PHP %s', $overload_option))
        ->setMessage($message)
    ...
    if (strlen($open_basedir)) {
      ...
      $issue = $this->newIssue('php.open_basedir')
        ->setName(\Yii::t("app",'Disable PHP %s', 'open_basedir'))
