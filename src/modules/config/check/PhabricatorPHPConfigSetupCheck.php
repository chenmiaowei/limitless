  ...
  protected function executeChecks() {
    ...
    if (empty($_SERVER['REMOTE_ADDR'])) {
      ...
      $message = \Yii::t("app",
        ...
        phutil_tag(
          ...
          \Yii::t("app",'Configuring a Preamble Script')));

      $this->newIssue('php.remote_addr')
        ->setName(\Yii::t("app",'No REMOTE_ADDR available'))
        ->setSummary($summary)
        ->setMessage($message);
    }
    ...
    if ($raw_post_data != -1) {
      ...
      $this->newIssue('php.always_populate_raw_post_data')
        ->setName(\Yii::t("app",'Disable PHP %s', 'always_populate_raw_post_data'))
        ->setSummary($summary)
        ->setMessage($message)
    ...
    if (!extension_loaded('mysqli')) {
      ...
      $this->newIssue('php.mysqli')
        ->setName(\Yii::t("app",'MySQLi Extension Not Available'))
        ->setSummary($summary)
        ->setMessage($message);
      ...
      $this->newIssue('php.myqlnd')
        ->setName(\Yii::t("app",'MySQL Native Driver Not Available'))
        ->setSummary($summary)
        ->setMessage($message);
    }
