  ...
  private function executeRefChecks(PhabricatorDatabaseRef $ref) {
    ...
    if ($max_allowed_packet < $recommended_minimum) {
      ...
      $this->newIssue('mysql.max_allowed_packet')
        ->setName(\Yii::t("app",'Small MySQL "%s"', 'max_allowed_packet'))
        ->setMessage($message)
        ->setDatabaseRef($ref)
    ...
    if (!in_array('STRICT_ALL_TABLES', $modes)) {
      ...
      $this->newIssue('mysql.mode')
        ->setName(\Yii::t("app",'MySQL %s Mode Not Set', 'STRICT_ALL_TABLES'))
        ->setSummary($summary)
        ->setMessage($message)
        ->setDatabaseRef($ref)
    ...
    if (in_array('ONLY_FULL_GROUP_BY', $modes)) {
      ...
      $this->newIssue('mysql.mode')
        ->setName(\Yii::t("app",'MySQL %s Mode Set', 'ONLY_FULL_GROUP_BY'))
        ->setSummary($summary)
        ->setMessage($message)
        ->setDatabaseRef($ref)
        ->addMySQLConfig('sql_mode');
    ...
    if ($is_myisam_fulltext) {
      ...
      if ($stopword_file === null) {
        ...
        $this->newIssue('mysql.ft_stopword_file')
          ->setName(\Yii::t("app",'MySQL %s Not Supported', 'ft_stopword_file'))
          ->setSummary($summary)
          ->setMessage($message)
          ->setDatabaseRef($ref)
        ...
        $this->newIssue('mysql.ft_stopword_file')
          ->setName(\Yii::t("app",'MySQL is Using Default Stopword File'))
          ->setSummary($summary)
          ->setMessage($message)
          ->setDatabaseRef($ref)
          ->addMySQLConfig('ft_stopword_file');
    ...
    if ($is_myisam_fulltext) {
      ...
      if ($min_len >= 4) {
        ...
        $this->newIssue('mysql.ft_min_word_len')
          ->setName(\Yii::t("app",'MySQL is Using Default Minimum Word Length'))
          ->setSummary($summary)
          ->setMessage($message)
          ->setDatabaseRef($ref)
    ...
    if ($innodb_bytes < $minimum_bytes) {
      ...
      $this->newIssue('mysql.innodb_buffer_pool_size')
        ->setName(\Yii::t("app",'MySQL May Run Slowly'))
        ->setSummary($summary)
        ->setMessage($message)
        ->setDatabaseRef($ref)
    ...
    if (!$ok) {
      ...
      $this->newIssue('mysql.utf8mb4')
        ->setName(\Yii::t("app",'Old MySQL Version'))
        ->setSummary($summary)
        ->setDatabaseRef($ref)
        ->setMessage($message);
    }
    ...
    if ($delta > 60) {
      $this->newIssue('mysql.clock')
        ->setName(\Yii::t("app",'Major Web/Database Clock Skew'))
        ->setSummary(
          \Yii::t("app",
