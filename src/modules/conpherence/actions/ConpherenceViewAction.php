  ...
  private function renderFormContent() {
    ...
    if ($user->isLoggedIn()) {
      ...
      if ($participating) {
        ...
        $status = \Yii::t("app",'Sending a message will also join the room.');
      }
      ...
      return (new PHUIFormLayoutView())
        ...
        ->appendInstructions(\Yii::t("app",'Log in to join this room and participate.'))
        ->appendChild(
          (new PHUIButtonView())
          ->setTag('a')
          ->setText(\Yii::t("app",'Log In to Participate'))
