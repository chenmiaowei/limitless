...
final class PhabricatorOpcodeCacheSpec extends PhabricatorCacheSpec {
  ...
  private function initAPCSpec() {
    ...
    if (ini_get('apc.enabled')) {
      ...
      if (!$write_lock || $slam_defense) {
        $summary = \Yii::t("app",'Adjust APC settings to quiet unnecessary errors.');
        ...
        $this
          ...
          ->setShortName(\Yii::t("app",'Noisy APC'))
          ->setName(\Yii::t("app",'APC Has Noisy Configuration'))
          ->setSummary($summary)
          ->setMessage($message)
      ...
      if ($is_stat_enabled && !$is_dev) {
        ...
        $this
          ...
          ->setShortName(\Yii::t("app",'"%s" Enabled', 'apc.stat'))
          ->setName(\Yii::t("app",'"%s" Enabled in Production', 'apc.stat'))
          ->setSummary($summary)
          ->setMessage($message)
        ...
        $this
          ...
          ->setShortName(\Yii::t("app",'"%s" Disabled', 'apc.stat'))
          ->setName(\Yii::t("app",'"%s" Disabled in Development', 'apc.stat'))
          ->setSummary($summary)
          ->setMessage($message)
          ->addPHPConfig('apc.stat')
          ->addPhabricatorConfig('phabricator.developer-mode');
  ...
  private function initOpcacheSpec() {
    $this
      ->setName(\Yii::t("app",'Zend OPcache'))
    ...
    if (ini_get('opcache.enable')) {
      ...
      if ($is_dev && (!$validate || $freq)) {
        ...
        $this
          ...
          ->setShortName(\Yii::t("app",'OPcache Config'))
          ->setName(\Yii::t("app",'OPcache Not Configured for Development'))
          ->setSummary($summary)
          ->setMessage($message)
        ...
        $summary = \Yii::t("app",'OPcache is not configured ideally for production.');
        ...
        $this
          ...
          ->setShortName(\Yii::t("app",'OPcache Config'))
          ->setName(\Yii::t("app",'OPcache Not Configured for Production'))
          ->setSummary($summary)
          ->setMessage($message)
          ->addPHPConfig('opcache.validate_timestamps')
          ->addPhabricatorConfig('phabricator.developer-mode');
      ...
      $summary = \Yii::t("app",'Enabling OPcache will dramatically improve performance.');
      $message = \Yii::t("app",
      ...
      $this->newIssue('extension.opcache.enable')
        ->setShortName(\Yii::t("app",'OPcache Disabled'))
        ->setName(\Yii::t("app",'Zend OPcache Not Enabled'))
        ->setSummary($summary)
        ->setMessage($message)
  ...
  private function initNoneSpec() {
    if (version_compare(phpversion(), '5.5', '>=')) {
      ...
      $this
        ...
        ->setShortName(\Yii::t("app",'OPcache'))
        ->setName(\Yii::t("app",'Zend OPcache Not Installed'))
        ->setMessage($message)
