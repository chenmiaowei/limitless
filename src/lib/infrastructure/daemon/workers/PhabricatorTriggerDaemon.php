 ...
 */
final class PhabricatorTriggerDaemon extends PhabricatorDaemon
{
...
//            $sleep_duration = $this->runCalendarNotifier($sleep_duration);
    ...
    private function scheduleTriggers($cursor)
    {
        ...
        $query = PhabricatorWorkerTrigger::find()
        ...
        while (true) {
            ...
            foreach ($triggers as $trigger) {
                ...
                // trigger is updated.
                if(!$new_event->save()) {
                    throw new ActiveRecordException("trigger event save error. ", $new_event->getErrorSummary(true));
                }
            ...
            /** @var PhabricatorWorkerTrigger $wild */
            $wild = last($triggers);
            $min = $wild->getTriggerVersion() + 1;
    ...
    private function getSleepDuration()
    {
        ...
        $next_triggers = PhabricatorWorkerTrigger::find()
            ...
            ->execute();
        if ($next_triggers) {
            /** @var PhabricatorWorkerTrigger $next_trigger */
    ...
    {
        ActiveRecord::overwriteCounterValue(
            (new PhabricatorWorkerTrigger())->getDb(),
            self::COUNTER_CURSOR,
            $value);
    }
    ...
    {
        return (int)ActiveRecord::loadCurrentCounterValue((new PhabricatorWorkerTrigger())->getDb(), $counter_name);
    }
