<?php
namespace orangins\lib\infrastructure\edges\editor;

use orangins\lib\infrastructure\edges\constants\PhabricatorEdgeConfig;
use orangins\lib\infrastructure\edges\exception\PhabricatorEdgeCycleException;
...
use orangins\lib\infrastructure\edges\util\PhabricatorEdgeGraph;
 ...
 */
final class PhabricatorEdgeEditor extends OranginsObject
{
     ...
     *
     * @param string  Source object PHID.
     * @param string Edge type constant.
     * @param string  Destination object PHID.
     * @param array   Options map (see documentation).
     * @return static
     *
     * @task edit
     * @throws Exception
    ...
    private function buildEdgeSpecs($src, $type, $dst, array $options = array())
    {
        ...
        $src_type = PhabricatorPHID::phid_get_type($src);
        $dst_type = PhabricatorPHID::phid_get_type($dst);
    ...
    private function executeAdds()
    {
        ...
        $inserts = array();
        foreach ($adds as $src_type => $edges) {
            $buildObject = PhabricatorEdgeConfig::buildObject($src_type);
            $conditions = array();
            foreach ($edges as $edge) {
                $conditions[] = [
                    ...
                    'seq' => $edge['seq'],
                    'data_id' => ArrayHelper::getValue($edge, 'data_id')
                ];
            }
            $inserts[] = array($buildObject, $conditions);
        }

        foreach ($inserts as $insert) {
            /** @var PhabricatorEdgeInterface $buildObject */
            list($buildObject, $conditions) = $insert;
            assert_instances_of([$buildObject], PhabricatorEdgeInterface::class);

            $buildObject->openTransaction();
            $this->openTransactions[] = $buildObject;
            $from = $buildObject->edgeBaseTableName() . "_" . PhabricatorEdgeConfig::TABLE_NAME_EDGE;
            foreach ($conditions as $condition) {
                \Yii::$app->getDb()->createCommand()->upsert($from, $condition)->execute();
            }
        }
    }
    ...
    private function executeRemoves()
    {
        ...
        foreach ($rems as $src_type => $edges) {
            ...
            $deletes[] = array($buildObject, $conditions);
        }

        foreach ($deletes as $delete) {
            /** @var PhabricatorEdgeInterface $buildObject */
            list($buildObject, $conditions) = $delete;
            assert_instances_of([$buildObject], PhabricatorEdgeInterface::class);
            ...
            $from = $buildObject->edgeBaseTableName() . "_" . PhabricatorEdgeConfig::TABLE_NAME_EDGE;
            foreach ($conditions as $condition) {
                $execute = \Yii::$app->getDb()->createCommand()->delete($from, $condition)->execute();
                \Yii::debug($execute);
            }
        }
    }
     ...
     * prevent cycles on.
     *
     * @return array<const> List of edge types which should have cycles prevented.
     * @task cycle
     * @throws Exception
     */
     ...
     * @return void
     * @task cycle
     * @throws PhabricatorEdgeCycleException
     */
    private function detectCycles(array $phids, $edge_type)
    {
        ...
        $graph = (new PhabricatorEdgeGraph())
