...
use orangins\lib\events\PhabricatorEventEngine;
use orangins\lib\exception\PhabricatorClusterStrandedException;
use orangins\lib\helpers\OranginsUtil;
use orangins\lib\infrastructure\internationalization\scope\PhabricatorLocaleScopeGuard;
...
use PhutilTranslation;
use PhutilTranslator;
use PhutilURI;
 ...
 */
final class PhabricatorEnv extends OranginsObject
{

    /**
     * @var PhabricatorConfigStackSource
     */
...
//        DarkConsoleEventPluginAPI::enableDiscardMode();
    ...
    private static function initializeCommonEnvironment($config_optional)
    {
        ...
        // invalid, use UTC.
        $tz = self::getEnvConfig('orangins.timezone');
        if ($tz) {
        ...
        $orangins_path = dirname(phutil_get_library_root('orangins'));
        $support_path = $orangins_path . '/support/bin';
    ...
    /**
     * @param $locale_code
     * @return PhabricatorLocaleScopeGuard
     * @throws \Exception
     */
    ...
    private static function buildConfigurationSourceStack($config_optional)
    {
        ...
        $default_source = (new PhabricatorConfigDefaultSource())
            ->setName(\Yii::t('app', 'Global Default'));
        $stack->pushSource($default_source);

        $env = self::getSelectedEnvironmentName();
        if ($env) {
            $stack->pushSource(
                (new PhabricatorConfigFileSource($env))
                    ->setName(\Yii::t('app', "File '{0}'", [$env])));
        }

        $stack->pushSource(
            (new PhabricatorConfigLocalSource())
                ->setName(\Yii::t('app', 'Local Config')));
        ...
        // pull in all options from non-orangins libraries now they are loaded.
        $default_source->loadExternalOptions();

        // If this install has site config sources, load them now.
        $site_sources = (new PhutilClassMapQuery())
            ->setAncestorClass(PhabricatorConfigSiteSource::className())
...
//        }
        ...
        try {
            $stack->pushSource(
                (new PhabricatorConfigDatabaseSource('default'))
                    ->setName(\Yii::t('app', 'Database')));
    ...
    public static function repairConfig($key, $value)
    {
        if (!self::$repairSource) {
            self::$repairSource = (new PhabricatorConfigDictionarySource(array()))
                ->setName(\Yii::t('app', 'Repaired Config'));
    ...
    public static function overrideConfig($key, $value)
    {
        if (!self::$overrideSource) {
            self::$overrideSource = (new PhabricatorConfigDictionarySource(array()))
                ->setName(\Yii::t('app', 'Overridden Config'));
    ...
    public static function getSelectedEnvironmentName()
    {
        ...
        $env = ArrayHelper::getValue($_SERVER, $env_var);
        ...
        if (!$env) {
            $env = ArrayHelper::getValue($_ENV, $env_var);
        }
    ...
    public static function getEnvConfig($key)
    {
        if (!self::$sourceStack) {
            throw new Exception(
                \Yii::t('app',
                    'Trying to read configuration "{0}" before configuration has been ' .
                    'initialized.',
                    [
                        $key
                    ]
        ...
        if (array_key_exists($key, $result)) {
            ...
            throw new Exception(
                \Yii::t('app',
                    "No config value specified for key '{0}.",
                    [$key]));
        }
    }
    ...
    {
        $str = self::getAnyBaseURI();
        return rtrim($str, '/') . $path;
    }
    ...
    public static function getProductionURI($path)
    {
        ...
        $production_domain = self::getEnvConfig('orangins.production-uri');
    ...
    private static function getSelfURIMap()
    {
        ...
        $allowed_uris = self::getEnvConfig('orangins.allowed-uris');
        ...
        foreach ($self_uris as $self_uri) {
            $host = (new PhutilURI($self_uri))->getDomain();
            if (!strlen($host)) {
                continue;
            }
    ...
    public static function newObjectFromConfig($key, $args = array())
    {
        ...
        return OranginsUtil::createObject($class, $args);
    }
    ...
    {
        $base_uri = self::getEnvConfig('orangins.base-uri');
        ...
        if (!$base_uri) {
            throw new Exception(
                \Yii::t('app',
                    "Define '{0}' in your configuration to continue.",
                    [
                        'orangins.base-uri'
                    ]));
        }
    ...
    public static function getReadOnlyMessage()
    {
        ...
        return \Yii::t('app', 'Phabricator is in read-only mode.');
    }
    ...
    public static function requireValidRemoteURIForLink($raw_uri)
    {
        ...
        if (!strlen($proto)) {
            throw new Exception(
                \Yii::t('app',
                    'URI "{0}" is not a valid linkable resource. A valid linkable ' .
        ...
        if (!isset($protocols[$proto])) {
            throw new Exception(
                \Yii::t('app',
                    'URI "{0}" is not a valid linkable resource. A valid linkable ' .
                    'resource URI must use one of these protocols: {1}.',
                    [
                        $raw_uri,
     ...
     *
     * @param string URI to test.
     * @param array<string> Allowed protocols.
        ...
        array $protocols)
    {
        ...
        if (!strlen($proto)) {
            throw new Exception(
                \Yii::t('app',
                    'URI "{0}" is not a valid fetchable resource. A valid fetchable ' .
                    'resource URI must specify a protocol.',
                    [
                        $raw_uri
                    ]));
        ...
        if (!$addresses) {
            throw new Exception(
                \Yii::t('app',
                    'URI "{0}" is not a valid fetchable resource. The domain "{1}" could ' .
                    'not be resolved.',
                    [
                        $raw_uri,
                        $domain
        ...
        foreach ($addresses as $address) {
            if (self::isBlacklistedOutboundAddress($address)) {
                throw new Exception(
                    \Yii::t('app',
                        'URI "{0}" is not a valid fetchable resource. The domain "{1}" ' .
                        'resolves to the address "{2}", which is blacklisted for ' .
                        'outbound requests.',
                        [
                            $raw_uri,
                            $domain,
        ...
        $resolved_uri->setDomain(OranginsUtil::head($addresses));
    ...
    /**
     * @return PhutilIPAddress
     */
    public static function getRemoteAddress()
    {
        $address = ArrayHelper::getValue($_SERVER, 'REMOTE_ADDR');
        if (!$address) {
            return null;
        }
    ...
    /**
     * @return PhabricatorConfigStackSource
     */
