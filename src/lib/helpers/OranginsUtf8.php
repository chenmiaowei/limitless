 ...
 */
class OranginsUtf8 extends OranginsObject
{
     ...
     *                  U+FFFD.
     */
    public static function phutil_utf8ize($string)
    {
        if (self::phutil_is_utf8($string)) {
            return $string;
        }
     ...
     */
    public static function phutil_is_utf8_with_only_bmp_characters($string)
    {
        return self::phutil_is_utf8_slowly($string, $only_bmp = true);
    }
    ...
    public static function phutil_is_utf8($string)
    {
        ...
        return self::phutil_is_utf8_slowly($string);
    }
     ...
     * @return bool   True if the string is valid UTF-8.
     */
    public static function phutil_is_utf8_slowly($string, $only_bmp = false)
    {
     ...
     * @throws Exception
     */
    public static function phutil_utf8_strlen($string)
    {
        if (function_exists('utf8_decode')) {
            return strlen(utf8_decode($string));
        }
        return count(self::phutil_utf8v($string));
    }
     ...
     * @throws Exception
     */
    public static function phutil_utf8_console_strlen($string)
    {
        ...
        $display_glyphs = self::phutil_utf8v_combined($string);
        foreach ($display_glyphs as $display_glyph) {
            $glyph_codepoints = self::phutil_utf8v_codepoints($display_glyph);
     ...
     * @throws Exception
     */
    public static function phutil_utf8_is_cjk($string)
    {
        $codepoints = self::phutil_utf8v_codepoints($string);
     ...
     * @throws Exception
     */
    public static function phutil_utf8v($string, $byte_limit = null)
    {
        $res = array();
        $len = strlen($string);
        ...
        while ($ii < $len) {
            ...
            if ($byte <= "\x7F") {
                ...
                throw new Exception(
                    \Yii::t("app", 'Invalid UTF-8 string passed to {0}.', [
                        __FUNCTION__
                    ]));
            } else if ($byte <= "\xDF") {
                $seq_len = 2;
     ...
     *
     * @param   string  A valid UTF-8 string.
     * @return array A list of codepoints, as integers.
     * @throws Exception
     */
    public static function phutil_utf8v_codepoints($string)
    {
        $str_v = self::phutil_utf8v($string);
     ...
     * @throws Exception
     */
    public static function phutil_utf8_encode_codepoint($codepoint)
    {
        if ($codepoint < 0x80) {
            $r = chr($codepoint);
            ...
            throw new Exception(
                \Yii::t("app",
                    'Encoding UTF8 codepoint "{0}" is not supported.',
                    [
                        $codepoint
                    ]));
        }
     ...
     * @throws Exception
     */
    public static function phutil_utf8_hard_wrap_html($string, $width)
    {
        $break_here = array();

        // Convert the UTF-8 string into a list of UTF-8 characters.
        $vector = self::phutil_utf8v($string);
        $len = count($vector);
        $char_pos = 0;
        for ($ii = 0; $ii < $len; ++$ii) {
     ...
     * @throws Exception
     */
    public static function phutil_utf8_hard_wrap($string, $width)
    {
        $result = array();

        $lines = OranginsUtil::phutil_split_lines($string, $retain_endings = false);
        foreach ($lines as $line) {

            // Convert the UTF-8 string into a list of UTF-8 characters.
            $vector = self::phutil_utf8v($line);
     ...
     * @throws Exception
     */
    public static function phutil_utf8_convert($string, $to_encoding, $from_encoding)
    {
        if (!$from_encoding) {
        ...
        if (!function_exists('mb_convert_encoding')) {
            throw new Exception(
                \Yii::t("app",
                    "Attempting to convert a string encoding from '{0}' to '{1}', " .
                    "but the '{2}' PHP extension is not available. Install {3} to " .
        ...
        if ($result === false) {
            ...
            if ($message) {
                $message = ArrayHelper::getValue($message, 'message', \Yii::t("app", 'Unknown error.'));
            }
            throw new Exception(
                \Yii::t("app",
                    "String conversion from encoding '{0}' to encoding '{1}' failed: {2}",
                    [
                        $from_encoding,
                        $to_encoding,
                        $message
     ...
     * @throws Exception
     */
    public static function phutil_utf8_ucwords($str)
    {
        ...
        foreach ($v as $c) {
            ...
            if ($convert) {
                $result .= self::phutil_utf8_strtoupper($c);
            } else {
                $result .= $c;
            }
     ...
     * @throws Exception
     */
    public static function phutil_utf8_strtolower($str)
    {
        ...
        return self::phutil_utf8_strtr($str, $map);
    }
     ...
     * @throws Exception
     */
    public static function phutil_utf8_strtoupper($str)
    {
        if (function_exists('mb_convert_case')) {
     ...
     * @throws Exception
     */
    public static function phutil_utf8_strtr($str, array $map)
    {
        $v = self::phutil_utf8v($str);
        $result = '';
        foreach ($v as $c) {
     ...
     * @throws Exception
     */
    public static function phutil_utf8_is_combining_character($character)
    {
        $components = self::phutil_utf8v_codepoints($character);
     ...
     * @throws Exception
     */
    public static function phutil_utf8v_combined($string)
    {
        $components = self::phutil_utf8v($string);
        return self::phutil_utf8v_combine_characters($components);
    }
     ...
     * @return array List of UTF-8 strings with combining characters merged.
     * @throws Exception
     */
    public static function phutil_utf8v_combine_characters(array $characters)
    {
        if (!$characters) {
            return array();
        }
        ...
        // start with a space.
        if (self::phutil_utf8_is_combining_character($characters[0])) {
            $buf = ' ';
        } else {
            $buf = null;
        }
        ...
        foreach ($characters as $character) {
            if (!isset($character[1])) {
                ...
                $is_combining = self::phutil_utf8_is_combining_character($character);
            }
     ...
     * @throws Exception
     */
    public static function phutil_get_system_locale()
    {
     ...
     * @throws Exception
     */
    public static function phutil_is_system_locale_available($locale)
    {
        $old_locale = self::phutil_get_system_locale();
     ...
     * @throws Exception
     */
    public static function phutil_set_system_locale($locale)
    {
        $ok = @setlocale(LC_ALL, $locale);
        if (!$ok) {
            throw new Exception(
                \Yii::t("app",
                    'Failed to set system locale (to "{0}").',
                    [
                        $locale
                    ]));
        }
